...
// [1] 각 항목들을 압력받아 변수에 저장.
let 제품명 = prompt("추가할 제품명을 입력해주세요 : ");
let 가격 = prompt("가격을 입력해주세요. : ");
let 제조사 = prompt("제조사를 입력해주세요 : ");

// [2] 각 입력받은 값들이 저장된 변수들을 속성값에 대입하여 객체 선언 { "속성명" : 변수값 }
// 주의할 점 : 변수명과 속성명은 다르다.
let 제품 = { "제품명": 제품명, "가격": 가격, "제조사": 제조사 };
console.log(제품);
...
// 주의할 점 : 속성명은 저장할 배열 내 존재하는 객체들의 속성명과 동일하게 헤야 한다.
let member = { id : 아이디 , password : 비밀번호 , name : 이름 };


//===============만약에 입력받은 아이디가 배열 내에 존재하면 {경우의 수 발생}
// [*] 배열 내 모든 객체를 하나씩 조회 == 배열 조회( 0부터 마지막 인덱스 )
let find = false; // 중복여부 체크 변수 , true 중복 false 중복 아님 , 스위치 변수 = on/off = 참/거짓 = 맞다/틀리다.
for(let index = 0 ; index <= members.length-1 ; index++ ){
    let m = members[index]; // index번째 회원 정보(객체) 꺼내기
    if( m.id == member.id ){ // 만약에 index번째 회원 정보의 아이디가 입력받은 아이디와 같으면
        // console.log("중복입니다."); // 불가능 , 아직 뒤에 있는 인덱스까지 확인 안 했으므로 최종 결론 불가능
        // 최종 결론 내지 말고 중복을 찾았다 못 찾았다만 기록 , 결론은 반복문 끝나고 하자.
        find = true; // 찾았다고 기록하고
        break; // 반복문 탈출
    }
}
...
}else{console.log("존재하는 아이디입니다.");} // [4] 배열 내 전체 출력
...
for(let index = 0 ; index <= scores.length-1 ; index++){
    sum3 += scores[index].math; // index번째의 수학점수를 누적합계하기.
} // for END
...
let check = false; // false 못 찾았다 true 찾았다
...
/* 주의할 점 : 왼쪽은 문제 2 , 오른쪽은 문제 4 에서 표현한 방식이지만 둘 다 답이 같다.
...
const isActive = [];
// [1] 새로운 배열 , 빈 배열 하나 선언
for( let index = 0 ; index <= users.length-1 ; index++ ){ // [2] 배열/목록에서 isActive가 true인 사용자들 찾기 == 배열/목록 순호;
if(userspindex[.isActive == true]){ // [3] 만약에 index 객체 내 isActive가 true 이면
    activeUsers.push( users[index]); // 새로운 배열에 index번째 객체 추기
}// if END
}// for END
...
for(let index = 0 ; index <= movies.length-1 ; index++){ // [2] 배열/목록에서 순회하여 각 title 조회하기
    movieTitles.push(movies[index].title); // index 번째 내 title 속성값을 새로운 배열에 추가(push)
}
...
// '기획팀': ['영희', '지혜']
// }
const result={}; // 결과가 담길 빈 객체를 일단 생성. 

// let : 바구니 자체를 내다버리고 새 바구니로 갈아치울 때 사용
// const : 바구니는 딱 고정해두고, 그 안에 사과나 배를 넣었다 뺐다 할 때 사용
// 우리가 만ㄷ는 result는 객체 {}다. result라는 바구니 자체를 다른 걸로 바꿀 생각 X 
// 그 내부에 있는 방을 만들거나 이름을 추가하는 것 뿐.
